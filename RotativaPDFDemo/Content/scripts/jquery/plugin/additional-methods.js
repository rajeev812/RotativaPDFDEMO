...
jQuery.validator.addMethod("password", function( value, element ) {
	var result = this.optional(element) ||  /^(?=^[A-Z])(?=^.{8,16}$)(?=.*[~!@#$%^&*()_+={}\[\]|:;<>,./?-])(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?!^.*\n).*$/.test(value);
	return result;
	}, "Password must contain at least one lower case, one upper case and one number. Spaces are not allowed. Length at least 8 characters .  Must start with capital character.  Must contain at least 1 non-character (such as !,#,%,@, etc). ");

jQuery.validator.addMethod("notEqualTo", function(value, element, param) {
	return this.optional(element) || value != $(param).val();
}, "This has to be different from the above value");
